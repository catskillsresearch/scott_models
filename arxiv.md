...
| ----------------------------------------- | ------------------------------ | ------------------------------------------ | -------------------------------- |
| `continuousLattice_to_neighborhoodSystem` | 1972 → 1980                    | 1972 **2.11**, **2.12**; Δ as master set | **Pass** — `↟a` nbhds; `D ≃o RoundFilter` (`domainOrderIso`); non-round filters properly larger (`ContinuousLatticeToNeighborhood.lean`); axioms ⊆ `{propext, Quot.sound}` |
...
| `presentation_domains_equiv`              | I ↔ II ↔ III                   | all above                                  | **Pass** — `D ≃o RoundFilter ≃o RoundInfoSysElement` via `wayBelowNbhdBasis` (`PresentationDomains.lean`); raw `\|𝒟\|`/`\|A\|` larger; axioms ⊆ `{propext, Quot.sound}` |
